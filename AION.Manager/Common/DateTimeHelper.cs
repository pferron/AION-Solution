...
namespace AION.BL.Common
{
    public class DateTimeHelper
    {
        ...
        public static List<DateTime> GetBlockedHolidayDatesForExpress(int year)
        {
            List<DateTime> standardHolidays = GetBlockedHolidaysForExpress(year);
            // need to be able to cross over into following year
            standardHolidays.AddRange(GetBlockedHolidaysForExpress(year + 1));
            return standardHolidays;
        }

        public static List<DateTime> GetBlockedHolidaysForExpress(int year)
        {
        ...
        public static DateTime DetermineWorkDateAfterDateSpecified(DateTime startDate, int numOfDays)
        {
            DateTime dateInAdvance = new DateTime();
            IHolidayConfigAdapter holidayConfigAdapter = new HolidayConfigAdapter();

            var holidayDates = holidayConfigAdapter.GetHolidayConfigDates();

            for (int i = 1; i <= numOfDays; i++)
            {
                dateInAdvance = startDate.AddDays(i);

                if ((dateInAdvance.DayOfWeek == DayOfWeek.Saturday) || (dateInAdvance.DayOfWeek == DayOfWeek.Sunday))
                {
                    numOfDays++;
                    continue;
                }
                if (CheckIfDateIsHoliday(dateInAdvance, holidayDates))
                {
                    numOfDays++;
                    continue;
                }
            ...
            return dateInAdvance;
        }

        public static DateTime DetermineWorkDateBeforeDateSpecified(DateTime startDate, int numOfDays)
        {
            DateTime dateInAdvance = new DateTime();
            IHolidayConfigAdapter holidayConfigAdapter = new HolidayConfigAdapter();
            ...
            if (dateInAdvance.Date < DateTime.Now.Date)
            {
        ...
        private static bool CheckIfDateIsHoliday(DateTime date, List<DateTime> holidays)
        {
            bool isHoliday = holidays.Any(d => d.Month == date.Month && d.Day == date.Day);
            return isHoliday;
        }

        /// <summary>
        /// Get the number of working days between two dates
        /// excludes weekends and holidays from the holiday config table
        /// used by SchedulingLeadTimeReportBO
        /// </summary>
        /// <param name="startDate"></param>
        /// <param name="endDate"></param>
        /// <returns></returns>
        public static int GetWorkDayCountFromDateRange(DateTime startDate, DateTime endDate)
        {
            int numOfDays = 0;
            ...
            for (var day = startDate.Date; day <= endDate; day = day.AddDays(1))
            {
                ...
                if (CheckIfDateIsHoliday(day, holidayDates))
                {
                    continue;
                }

                //add to the count if not weekend or holiday
                numOfDays++;
