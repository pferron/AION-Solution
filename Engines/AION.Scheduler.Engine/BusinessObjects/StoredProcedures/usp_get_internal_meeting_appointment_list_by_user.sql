/***********************************************************************************************************************          
* Object:       usp_get_internal_meeting_appointment_list_by_user          
...
* 12/15/2020    jlindsay    fix status search        
* 4/16/2021  jlindsay add project schedule types      
* 4/30/2021 jlindsay add project fields for dashboard          
* 07/08/2021 jlindsay add new field for Accela record id REC_ID_TXT  
* 09/17/2021 jallen add new field for Accela record id TEAM_GRADE_TXT  
* 11/3/2021 jlindsay update for new sub cycle integration
***********************************************************************************************************************/
ALTER PROCEDURE [AION].[usp_get_internal_meeting_appointment_list_by_user] @identity INT
AS
BEGIN
	...
	INNER JOIN [AION].[USER_SCHEDULE] US ON U.[USER_Id] = US.[USER_ID]
	INNER JOIN [AION].[PROJECT_SCHEDULE] PS ON US.[PROJECT_SCHEDULE_ID] = PS.[PROJECT_SCHEDULE_ID]
	INNER JOIN [AION].[PLAN_REVIEW_SCHEDULE] EMA ON PS.APPT_ID = EMA.PLAN_REVIEW_SCHEDULE_ID
		AND PS.[PROJECT_SCHEDULE_TYP_DESC] = 'EMA'
	INNER JOIN [AION].[PROJECT_CYCLE] PC ON PC.PROJECT_CYCLE_ID = EMA.PROJECT_CYCLE_ID
	INNER JOIN [AION].[PROJECT] P ON P.[PROJECT_ID] = PC.PROJECT_ID
	INNER JOIN [AION].[PROJECT_STATUS_REF] PSR ON PSR.PROJECT_STATUS_REF_ID = P.PROJECT_STATUS_REF_ID
	...
	SELECT DISTINCT PS.[RECURRING_APPT_DT] AS 'NULL',
		9 AS [MEETING_TYPE],
		P.[PROJECT_TYP_REF_ID] AS [PROJECT_TYPE_ID], --DO NOT HAVE ENUM MAP AND ENUMS ARE MAPPED DIRECTLY TO PK.           
		P.[SRC_SYSTEM_VAL_TXT] AS [PROJECT_EXTREF_ID],
		P.[PROJECT_NM] AS [PROJECT_NM],
		...
		P.[PROJECT_ID] AS [PROJECT_ID],
		U.[USER_ID] AS [USER_ID],
		FS.[WKR_ID_CREATED_TXT] AS [WKR_ID_CREATED_TXT],
		FS.[CREATED_DTTM] AS [CREATED_DTTM],
		FS.[WKR_ID_UPDATED_TXT] AS [WKR_ID_UPDATED_TXT],
		FS.[UPDATED_DTTM] AS [UPDATED_DTTM],
		NULL AS [APPT_CANCELLATION_REF_ID],
		P.[RTAP_IND],
		P.[ASSIGNED_FACILITATOR_ID],
		P.[PROJECT_MANAGER_ID],
		P.REC_ID_TXT,
	...
	INNER JOIN [AION].[USER_SCHEDULE] US ON U.[USER_Id] = US.[USER_ID]
	INNER JOIN [AION].[PROJECT_SCHEDULE] PS ON US.[PROJECT_SCHEDULE_ID] = PS.[PROJECT_SCHEDULE_ID]
	INNER JOIN [AION].[PLAN_REVIEW_SCHEDULE] FS ON PS.APPT_ID = FS.PLAN_REVIEW_SCHEDULE_ID
		AND PS.[PROJECT_SCHEDULE_TYP_DESC] = 'FIFO'
	INNER JOIN [AION].[PROJECT_CYCLE] PC ON PC.PROJECT_CYCLE_ID = FS.PROJECT_CYCLE_ID
	INNER JOIN [AION].[PROJECT] P ON P.[PROJECT_ID] = PC.PROJECT_ID
	INNER JOIN [AION].[PROJECT_STATUS_REF] PSR ON PSR.PROJECT_STATUS_REF_ID = P.PROJECT_STATUS_REF_ID
	INNER JOIN [AION].[APPOINTMENT_RESPONSE_STATUS_REF] ARSR ON ARSR.APPT_RESPONSE_STATUS_REF_ID = FS.APPT_RESPONSE_STATUS_REF_ID
	WHERE U.[USER_ID] = @identity
		-- AND PS.[RECURRING_APPT_DT] IS NOT NULL        
		AND FS.APPT_RESPONSE_STATUS_REF_ID NOT IN (
			@closedapptstatus,
			@cancelledapptstatus
			)
		AND P.PROJECT_STATUS_REF_ID <> @CancelledProjectStatus;
