...
{
    public static class CancelReserveExpressReservationTrigger
    {
        ...
        [FunctionName("CancelReserveExpressReservationTrigger")]
        public static void Run([TimerTrigger("0 00 23 * * 1-5")] TimerInfo myTimer, TraceWriter log, ExecutionContext context) //Every Weekday 11PM
        {
            ...
            logmessage = $"Timer trigger function CancelReserveExpressReservationTrigger executed at: {DateTime.Now}";
            log.Info(logmessage);
            ...
            try
            {
                if (APIHelper.CancelReserveExpressReservation() == false)
                {
                    logmessage = "Error occurred during CancelReserveExpressReservationTrigger - APIHelper.CancelReserveExpressReservation() = false.";
                    throw new Exception(logmessage);
                }
            }
            catch (Exception ex)
            {
                logmessage = "Error occurred during CancelReserveExpressReservationTrigger. Check the logs for more details. ApplicationMessageGuid:" + applicationMessageGuid + ", Message: " + ex.Message + ", StackTrace: " + ex.StackTrace;
