...
* 03/24/2021 jlindsay remove dynamic sql, add all fields for projectBE, generated this version from usp_select_existing_projects_by_externalRefId      
* 04/21/2021 jlindsay add filter for project status enums      
...
***********************************************************************************************************************/
ALTER PROCEDURE [AION].[usp_select_existing_projects_by_externalRefId_splitIds] @prjSrcTxtValArr VARCHAR(MAX),
	@prjStatusEnums VARCHAR(500)
AS
BEGIN
	WITH ProjectStatusRef
	AS (
		--get the enum status ids from the enum_mapping_val_nbr list variable @prjStatusEnums      
		SELECT project_status_ref_id
		FROM [AION].PROJECT_STATUS_REF
		WHERE enum_mapping_val_nbr IN (
				SELECT [value]
	...
	FROM [AION].[PROJECT] p
	INNER JOIN ProjectStatusRef r ON p.[PROJECT_STATUS_REF_ID] = r.[PROJECT_STATUS_REF_ID]
	WHERE SRC_SYSTEM_VAL_TXT IN (
			SELECT [value]
			FROM STRING_SPLIT(@prjSrcTxtValArr, ',')
			)
